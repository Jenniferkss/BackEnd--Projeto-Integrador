...
export const criar = async (req, res) => {
    try {
        ...
        const { nome, curso, objetivoPt, objetivoEn } = req.body;

        if (!nome) {
            return res.status(400).json({ error: 'O campo "nome" é obrigatório para uma equipe!' });
        }

        if (!curso) {
            return res
                .status(400)
                .json({ error: 'O campo "curso" é obrigatório para uma equipe!' });
        }

        if (!objetivoPt) {
            return res
                .status(400)
                .json({ error: 'O campo "objetivoPt" é obrigatório para uma equipe!' });
        }

        if (!objetivoEn) {
            return res
                .status(400)
                .json({ error: 'O campo "objetivoEn" é obrigatório para uma equipe!' });
        }
...
export const atualizar = async (req, res) => {
    try {
        ...
        if (req.body.objetivoPt !== undefined) {
            equipe.objetivoPt = req.body.objetivoPt;
        }
